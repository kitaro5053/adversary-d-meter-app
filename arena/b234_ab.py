# -*- coding: utf-8 -*-
"""B-234 A/B ドライバ：ご神木の両方向演繹（`agents.belief.B234_GOSHINBOKU_TRAIT`）を比較する。

`arena/b231_ab.py` と同型。切替口は `agents.belief` のモジュールフラグ＝退避→上書き→復元。
`--perm`（id/rev/h1）は `arena.tie_noise.install_perm`（B-218 教訓＝perm 3条件）。
`--only`（`name#seed` のカンマ区切り）で対象局を絞れる＝発火局（ご神木∈cast）は
両ベンチで 21局だけで、フラグは `_GOSHINBOKU in self.cast` で短絡するため
**非該当局は構造的に bit 不変**（`agents/belief.py` `_recompute` の分岐参照）。

CLI:
    PYTHONHASHSEED=0 PYTHONIOENCODING=utf-8 python -m arena.b234_ab --days 3 \
        --conds off,on --perm id --out docs/仮_b234_log/ab_d3_id.json
"""
...
CONDS: dict[str, dict] = {
    "off": {},
    "on": {"B234_GOSHINBOKU_TRAIT": True},
    # ★B-231（妹の特性・land 済み既定 OFF）との相互作用を見るための条件。
    #   B-231 は 5日級 id の `random_BTX#3` で防衛 −1 を出した局＝B-234 が +1 を出す局と同一。
    "b231": {"B231_IMOUTO_TRAIT": True},
    "both": {"B231_IMOUTO_TRAIT": True, "B234_GOSHINBOKU_TRAIT": True},
}
_ATTRS = ("B234_GOSHINBOKU_TRAIT", "B231_IMOUTO_TRAIT")
...
def run_cond(name: str, days: int, loops: int = 8, perm: str = "id",
    ...
    # ★ベースライン測定（B-234 導入前のコミット）でも同じドライバを使えるように、
    #   フラグが存在しないツリーでは退避も上書きもしない（"off" 条件＝素の挙動）。
    old = {a: getattr(bl, a) for a in _ATTRS if hasattr(bl, a)}
    for k, v in CONDS[name].items():
        if not hasattr(bl, k):
            raise SystemExit(f"このツリーに {k} が無い＝'{name}' 条件は測れない")
        setattr(bl, k, v)
    install_perm(perm)
    rows = []
    try:
