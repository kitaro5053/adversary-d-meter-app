# -*- coding: utf-8 -*-
"""B-240 flip 検死ヘルパ（読み取り専用）：2条件で同一局を回して**最初の差分**を出す。

`arena/b237_diff.py` の 6切替口版（`b240` を足しただけ）。フラグは実行時に
退避→上書き→復元するだけ（ファイル不変）。条件名の書式は b236/b237 と同じ。
`base`＝現行既定（`b230`/`b231`/`b232`/`b234` の4つ ON）。

CLI:
    PYTHONHASHSEED=0 PYTHONIOENCODING=utf-8 python -m arena.b240_diff \
        --game random_BTX#1 --days 5 --perm id --a base --b base+b240 --limit 8
"""
...
ORDER = ("b230", "b231", "b232", "b234", "b237", "b240")
...
def _apply(knobs, extra: dict | None = None) -> dict:
    ...
    knob_audit.check_baseline(KNOBS, _holders(), BASE, driver="b240_diff")
    holders = _holders()
    on = set(knobs)
    old = {}
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="B-240 flip 検死（読み取り専用）")
    ap.add_argument("--game", required=True)
    ap.add_argument("--days", type=int, default=5)
    ap.add_argument("--perm", default="id")
    ap.add_argument("--a", default="base")
    ap.add_argument("--b", default="base+b240")
    ...
    extra = {"B240_CAP": a.cap} if a.cap is not None else None
