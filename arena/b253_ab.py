# -*- coding: utf-8 -*-
"""B-253 A/B ドライバ（★測定専用・実装も既定値も一切触らない）。

`arena/b247_ab.py` と同型（フラグは実行時に退避→上書き→復元＝ファイルは不変）。

条件に使える切替口（★**既定 OFF のものだけ**を載せる＝b247_ab の注意書きと同じ不変条件）：
- `gw` ＝ `HeuristicProtagonist.B253_PREFER_GOODWILL`（不安+1(キャラ)→友好 の振り替え）
- `vo` ＝ `HeuristicProtagonist.B253_VIRUS_ONLY`（ウイルス試験の席だけに限る狭い述語）

`--floor` は `_B253_GW_FLOOR`（float＝掃引口）を**非 off 条件にだけ**適用する。

CLI:
    PYTHONHASHSEED=0 PYTHONIOENCODING=utf-8 python -m arena.b253_ab \
        --days 3 --conds off,gw,gw+vo --out docs/仮_b253_log/ab_d3.json
"""
...
KNOBS: dict[str, tuple[str, str]] = {
    "gw": ("hp", "B253_PREFER_GOODWILL"),
    "vo": ("hp", "B253_VIRUS_ONLY"),
}
ORDER = ("gw", "vo")
_FLOOR_ATTR = "_B253_GW_FLOOR"
...
def _check_defaults() -> None:
    ...
    knob_audit.check_baseline(KNOBS, _holders(), (), driver="b253_ab")
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="B-253 A/B ドライバ（測定専用）")
    ap.add_argument("--days", type=int, default=3)
    ap.add_argument("--loops", type=int, default=8)
    ap.add_argument("--conds", default="off,gw,gw+vo")
