# -*- coding: utf-8 -*-
"""B-236 flip 検死ヘルパ（読み取り専用）：2条件で同一局を回して**最初の差分**を出す。

`arena/b235_diff.py` の 4切替口版（条件名は `arena.b236_ab` と同じ書式＝`b232+b234` 等）。
フラグは実行時に退避→上書き→復元するだけ（ファイル不変）。

CLI:
    PYTHONHASHSEED=0 PYTHONIOENCODING=utf-8 python -m arena.b236_diff \
        --game random_BTX#3 --days 5 --perm id --a b234 --b b231+b234 --limit 12 --roles
"""
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--b", default="b232+b234")
