# -*- coding: utf-8 -*-
"""B-235 flip 検死ヘルパ（読み取り専用）：2条件で同一局を回して**最初の差分**を出す。

フラグは実行時に退避→上書き→復元するだけ（ファイル不変）。
`--perm` は `arena.tie_noise.install_perm`（ベンチと同じ並び）。

CLI:
    PYTHONHASHSEED=0 PYTHONIOENCODING=utf-8 python -m arena.b235_diff \
        --game random_BTX#3 --days 5 --perm id --a off --b b231 --limit 12
"""
...
def _apply(b230: bool, b231: bool):
    ...
    HP.B230_SPLIT_GEOMETRY, bl.B231_IMOUTO_TRAIT = b230, b231
    return old
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="B-235 flip 検死（読み取り専用）")
    ap.add_argument("--game", required=True)
    ap.add_argument("--days", type=int, default=5)
    ap.add_argument("--perm", default="id")
    ap.add_argument("--a", default="off")
    ap.add_argument("--b", default="b231")
    ...
    print(f"# {a.game} days={a.days} perm={a.perm}: {a.a} vs {a.b}")
    print(f"  {a.a}: winner={sa.winner} loop={sa.loop_no} / "
          f"{a.b}: winner={sb.winner} loop={sb.loop_no}")
    if a.roles:
    ...
    while i < min(len(ka), len(kb)) and ka[i] == kb[i]:
        i += 1
    print(f"  一致した決定数={i} / {a.a}={len(la)} {a.b}={len(lb)}")
